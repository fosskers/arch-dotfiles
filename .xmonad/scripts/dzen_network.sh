...
essid=$(iwconfig wifi0 | sed -n "1p" | awk -F '"' '{print $2}')
mode=$(iwconfig wifi0 | sed -n "1p" | awk -F " " '{print $3}')
freq=$(iwconfig wifi0 | sed -n "2p" | awk -F " " '{print $2}' | cut -d":" -f2)
mac=$(iwconfig wifi0 | sed -n "2p" | awk -F " " '{print $6}')
qual=$(iwconfig wifi0 | sed -n "6p" | awk -F " " '{print $2}' | cut -d"=" -f2)
lvl=$(iwconfig wifi0 | sed -n "6p" | awk -F " " '{print $4}' | cut -d"=" -f2)
rate=$(iwconfig wifi0 | sed -n "3p" | awk -F "=" '{print $2}' | cut -d" " -f1)
inet=$(ifconfig wifi0 | sed -n "2p" | awk -F " " '{print $2}')
netmask=$(ifconfig wifi0 | sed -n "2p" | awk -F " " '{print $4}')
broadcast=$(ifconfig wifi0 | sed -n "2p" | awk -F " " '{print $6}')
